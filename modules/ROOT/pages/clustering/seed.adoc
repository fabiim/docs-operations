...
[NOTE]
====
The system database defines the databases that should exist in your neo4j instance.
If a database doesn't exist in your system database (because it hasn't been created previously) it will have to be created with `CREATE DATABASE <database-name>` even if it has been restored/loaded/imported.
====
